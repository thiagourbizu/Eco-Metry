...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(9), sens, RISING);  // hall sensor interrupt on GPIO 9
...
void sens() {
  if (millis() - lastturn > 80) {    // simple noise cut filter (to limit noise)
...
void loop() {
  ...
  if (digitalRead(8) == 1) {  // Si el botón está presionado
